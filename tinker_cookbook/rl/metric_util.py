...
def _compute_trajectory_metrics(trajectory_groups_P: List[TrajectoryGroup]) -> Dict[str, float]:
    ...
    # Filter out non-scalar metrics (e.g., lists, objects, strings) that shouldn't be averaged
    def filter_scalar_metrics(metrics_list):
        filtered = []
        for m in metrics_list:
            # Only keep numeric values (int, float, bool) that can be averaged
            # Note: bool is a subclass of int, so we check it separately
            filtered.append({k: v for k, v in m.items() if isinstance(v, (int, float, bool, np.number))})
        return filtered

    metrics.update(dict_mean(filter_scalar_metrics(transition_metrics + traj_metrics)))
    ...
    # Dual critique mode: track y2 accuracy separately for expert vs model paths
    expert_y2_metrics = []
    model_y2_metrics = []
    for m in transition_metrics:
        if m.get("path") == "expert" and "y2_correct" in m:
            expert_y2_metrics.append({"y2_correct": m["y2_correct"], "y2_format": m.get("y2_format", 0)})
        elif m.get("path") == "model" and "y2_correct" in m:
            model_y2_metrics.append({"y2_correct": m["y2_correct"], "y2_format": m.get("y2_format", 0)})

    if expert_y2_metrics:
        expert_stats = dict_mean(expert_y2_metrics)
        metrics["dual_critique/y2_expert_correct"] = expert_stats["y2_correct"]
        metrics["dual_critique/y2_expert_format"] = expert_stats["y2_format"]

    if model_y2_metrics:
        model_stats = dict_mean(model_y2_metrics)
        metrics["dual_critique/y2_model_correct"] = model_stats["y2_correct"]
        metrics["dual_critique/y2_model_format"] = model_stats["y2_format"]

    # Gap between expert and model performance
    if expert_y2_metrics and model_y2_metrics:
        metrics["dual_critique/y2_gap_correct"] = metrics["dual_critique/y2_expert_correct"] - metrics["dual_critique/y2_model_correct"]
        metrics["dual_critique/y2_gap_format"] = metrics["dual_critique/y2_expert_format"] - metrics["dual_critique/y2_model_format"]
...
class RLTestSetEvaluator(SamplingClientEvaluator):
    def __init__(
        ...
        self.dataset = dataset  # Store dataset reference for updating sampling client
    ...
    async def __call__(self, sampling_client: tinker.SamplingClient) -> dict[str, float]:
        # Update dataset with latest sampling client before evaluation
        # This is needed for datasets that use model self-critique (e.g., use_model_critique=True)
        if hasattr(self.dataset, 'update_sampling_client'):
            self.dataset.update_sampling_client(sampling_client)

...
class RLDatasetBuilder:
    ...
    @abstractmethod
    async def __call__(self) -> tuple[RLDataset, RLDataset | None | list[tuple[str, RLDataset]]]:
        """
        Return RLDataset (for training) and test dataset(s).

        Test datasets can be:
        - None: no test dataset
        - RLDataset: single test dataset (will use name "test")
        - list[tuple[str, RLDataset]]: multiple test datasets with custom names
        """
        pass

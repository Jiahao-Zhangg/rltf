...
@scope
def print_group(traj_group: TrajectoryGroup, tokenizer: Tokenizer, model_name: str = "none"):
    """
    ...
    # Check if trajectories have variable lengths (from early termination)
    trajectory_lengths = [len(traj.transitions) for traj in traj_group.trajectories_G]
    has_variable_length = len(set(trajectory_lengths)) > 1

    advantages_G = compute_advantages([traj_group], use_traj_return=has_variable_length, gamma=0.5) # 0.5 is just a placeholder

    data_D, metadata_D = assemble_training_data([traj_group], advantages_G, distillation_mode="none", model_name=model_name)
...
def compute_rl_coef(
    rl_coef_config: float | list[float],
    current_batch: int,
    total_batches: int,
    start_batch: int = 0,
    relative_schedule: bool = False,
) -> float:
    """
    Compute the current RL coefficient value based on training progress.

    Args:
        rl_coef_config: Either a float (constant) or [start, end] for linear schedule
        current_batch: Current batch index (0-indexed)
        total_batches: Total number of batches in training
        start_batch: Starting batch index (for resuming training)
        relative_schedule: If True, compute progress relative to start_batch (restart schedule on resume)
                          If False, compute progress using absolute batch index (default, continuous schedule)

    Returns:
        Current RL coefficient value
    """
    if isinstance(rl_coef_config, (int, float)):
        # Constant value
        return float(rl_coef_config)
    elif isinstance(rl_coef_config, list) and len(rl_coef_config) == 2:
        # Linear schedule from start to end
        start_val, end_val = rl_coef_config

        if relative_schedule:
            # Relative schedule: restart from 0 when resuming
            # Progress from start_batch to total_batches
            progress = (current_batch - start_batch) / max(total_batches - start_batch - 1, 1)
        else:
            # Absolute schedule: continue from where we left off (default behavior)
            progress = current_batch / max(total_batches - 1, 1)

        return start_val + (end_val - start_val) * progress
    else:
        raise ValueError(f"rl_coef must be a float or a list of 2 floats, got {rl_coef_config}")


def clipped_sft_loss(
        data: List[tinker.Datum],
        logprobs: List[torch.Tensor],
) -> tuple[torch.Tensor, dict[str, float]]:
    for datum, logprob in safezip(data, logprobs):
        weights = datum.loss_fn_inputs["weights"].to_torch()        
        mask = logprob > -0.05
        clipped_weights = torch.where(mask, weights, torch.zeros_like(weights))
        loss = -(clipped_weights * logprob).mean()
    metrics = {"clipped_sft_loss": loss.item()}
    return loss, metrics
...
async def forward_backward(
    ...
    if loss_fn in ["ppo", "cispo"]:
        loss_fn_config={"clip_low_threshold": clip_low, "clip_high_threshold": clip_high}
        fwd_bwd_future = await training_client.forward_backward_async(
            list(map(remove_mask, batch_d)), loss_fn=loss_fn, loss_fn_config=loss_fn_config
        )
    else:
        fwd_bwd_future = await training_client.forward_backward_async(
    ...
    return training_logprobs_D, fwd_bwd_result.metrics
...
async def train_step(
    ...
    for batch_d in batches_md:
        training_logprobs, metrics = await forward_backward(training_client, batch_d, loss_fn, clip_low, clip_high)
        training_logprobs_D.extend(training_logprobs)
        await optim_step(training_client, learning_rate)
        train_metrics[f"train/rl_loss"] = metrics["loss:sum"]
    return training_logprobs_D, train_metrics


@scope
async def train_step_combined(
    rl_data: List[tinker.Datum],
    ...
    """Train with combined RL and SFT loss: rl_loss + sft_weight * sft_loss."""
    # Scale SFT weights
    scaled_sft_data = []
    for datum in sft_data:
        original_weights = datum.loss_fn_inputs["weights"].to_torch()
        scaled_weights = original_weights * sft_weight
        
        scaled_datum = tinker.Datum(
            model_input=datum.model_input,
            loss_fn_inputs={
                **datum.loss_fn_inputs,
                "weights": tinker.TensorData.from_torch(scaled_weights)
            }
        )
        scaled_sft_data.append(scaled_datum)
    
    # Combine all data for batching
    all_data = rl_data + scaled_sft_data
    batches_md = split_list(all_data, min(num_substeps, len(all_data)))
    training_logprobs_D: list[torch.Tensor] = []

    train_metrics = {}
    
    for batch_d in batches_md:
        # Separate RL and SFT items in this batch
        batch_rl = [d for d in batch_d if "weights" not in d.loss_fn_inputs]
        batch_sft = [d for d in batch_d if "weights" in d.loss_fn_inputs]
        
        # Accumulate gradients from RL loss
        if batch_rl:
            rl_logprobs, metrics = await forward_backward(training_client, batch_rl, rl_loss_fn, clip_low, clip_high)
            training_logprobs_D.extend(rl_logprobs)
            train_metrics[f"train/rl_loss"] = metrics["loss:sum"]

        if batch_sft:
            sft_logprobs_D, sft_metrics = await forward_backward(training_client, batch_sft, "cross_entropy", clip_low, clip_high)
            # training_logprobs_D.extend(sft_logprobs_D)
            train_metrics[f"train/sft_loss"] = sft_metrics["loss:sum"]
        
        # Single optimizer step with combined gradients
        await optim_step(training_client, learning_rate)
...
class Config:
    ...
    eval_temperature: float | None = None  # Temperature for evaluation (None = use default)
    ...
    # Weight for combining SFT loss with RL loss (when using combined training)
    sft_loss_weight: float = 0.1
    # Number of training steps to keep KL advantages at 0 before switching to normal coefficient
    zero_kl_steps: int = 0
    # Start distillation only after this iteration (0 = start immediately)
    distillation_start_iteration: int = 0

    # SFT buffer parameters (for maintaining consistent SFT batch sizes)
    ...
    # Early termination and trajectory-level returns
    ...
    gamma: float = 0.1  # Discount factor: R_t = r_t + gamma*r_{t+1} + gamma^2*r_{t+2} + ...

    # PPO and ratio clipping thresholds (used in PPO loss and rl_ratio_clip distillation)
    ...
    # Distillation advantage baseline (for rl_reweight, rl_reweight_clip, rl_ratio_clip)
    use_first_turn_baseline: bool = False  # If True, use r_final - mean(r1); if False, use centered r2 advantage

    rl_coef: float | list[float] = 0.1  # Multiplier for advantages in RL distillation modes

    # If True, rl_coef schedule restarts from beginning when resuming (relative to start_batch)
    # If False, rl_coef schedule continues from where it left off (absolute batch index)
    relative_rl_coef_schedule: bool = False
...
async def do_sync_training_with_stream_minibatch(
    ...
    for i_batch in range(start_batch, end_batch):
        ...
        # Run evaluations
        if ((cfg.eval_every > 0 and i_batch > 0 and i_batch % cfg.eval_every == 0) or i_batch == end_batch - 1):
...
async def save_checkpoint_and_get_sampling_client(
    ...
    with timed("save_checkpoint", metrics):
        # OLD VERSION: Always saves to checkpoints.jsonl (either sampler-only or full checkpoint)
        # path_dict = await checkpoint_utils.save_checkpoint_async(
        #     training_client=training_client,
        #     name=f"{i_batch:06d}",
        #     log_path=log_path,
        ...
        # NEW VERSION: Only saves to checkpoints.jsonl when i_batch % save_every == 0
...
async def prepare_minibatch(
    ...
    for traj_group in trajectory_groups_P[:2]:
        print_group(traj_group, tokenizer, model_name=model_name)

    # Assemble training data
    with timed("assemble_training_data", metrics):
        # Normalize advantages by std for critique batches (log prob rewards have large magnitude)
        advantages_P = compute_advantages(
            trajectory_groups_P,
            normalize_by_std=is_critique_batch,
            gamma=cfg.gamma,
        )
        # Compute current rl_coef value (handles constant or scheduled)
        # Automatically use relative schedule when either start_batch or distillation_start_iteration is non-zero
        # Effective start is distillation_start_iteration if set, otherwise start_batch
        auto_relative = (start_batch > 0 or cfg.distillation_start_iteration > 0)
        use_relative = cfg.relative_rl_coef_schedule or auto_relative
        effective_start_batch = cfg.distillation_start_iteration if cfg.distillation_start_iteration > 0 else start_batch

        current_rl_coef = compute_rl_coef(
            cfg.rl_coef, i_batch, num_batches, effective_start_batch, use_relative
        )

        data_D, _metadata_D = assemble_training_data(
            trajectory_groups_P, advantages_P,
            distillation_mode=distillation_mode,
            sampling_client=sampling_client, model_name=model_name,
            rl_coef=current_rl_coef,
        )

        # Log current rl_coef if it's scheduled
        if isinstance(cfg.rl_coef, list):
            metrics["train/rl_coef"] = current_rl_coef
        
        

        # Extract clip_fraction from metadata for rl_reweight_clip
        if distillation_mode == "rl_reweight_mask" or distillation_mode == "rl_mask" and _metadata_D:
            distill_metadata = [m for m in _metadata_D if m.get("is_distillation", False)]
            if distill_metadata:
                mask_fractions = [m["mask_fraction"] for m in distill_metadata if "mask_fraction" in m]
                if mask_fractions:
                    metrics["train/mask_fraction"] = sum(mask_fractions) / len(mask_fractions)
...
async def do_train_step_and_get_sampling_client(
    ...
    sampling_client: tinker.SamplingClient | None = None,
    is_critique_batch: bool = False,
    sft_buffer: deque[tinker.Datum] | None = None,
    start_batch: int = 0,
) -> tuple[tinker.SamplingClient, dict[str, Any]]:
    update_scope_context({"step": i_batch})
    ...
    # Temporarily override distillation_mode if we haven't reached distillation_start_iteration
    if i_batch < cfg.distillation_start_iteration:
    ...
    data_D, prepare_minibatch_metrics = await prepare_minibatch(
        ...
        is_critique_batch=is_critique_batch,
        distillation_mode=distillation_mode,
        i_batch=i_batch,
    ...
    with timed("train", metrics):
        ...
        # Check if we need to combine RL and SFT data
        if distillation_mode in ["sft", "feedback_modeling"]:
            # Separate RL and SFT data based on loss_fn_inputs
            rl_data = []
            sft_data = []
            ...
            for datum in data_D:
                if "weights" in datum.loss_fn_inputs:
                    ...
                    rl_data.append(datum)

            # Add new SFT samples to buffer and use all buffered samples
            if cfg.sft_buffer_size > 0 and sft_buffer is not None:
                sft_buffer.extend(sft_data)

                # Use all samples from buffer to augment current batch
                if len(sft_buffer) > 0:
                    sft_data = list(sft_buffer)
                    logger.info(f"Added {len(sft_buffer)} samples from SFT buffer")

            logger.info(f"Training with separated data: {len(rl_data)} RL items, {len(sft_data)} SFT items (including buffered)")
            
            training_logprobs_D = []
            
            
            # Train with combined RL + weighted SFT loss
            training_logprobs_D, train_metrics = await train_step_combined(
                rl_data,
                sft_data,
                training_client,
                cfg.learning_rate,
                ...
                cfg.loss_fn,
                distillation_mode,
                sft_weight=cfg.sft_loss_weight,
                clip_low=cfg.clip_low,
                clip_high=cfg.clip_high,
            )
            metrics[f"train/num_rl_items"] = len(rl_data)
            metrics[f"train/num_sft_items"] = len(sft_data)
            metrics.update(train_metrics)
            
            # # Compute SFT NLL for logging if we have SFT data
            if sft_data:
                from tinker_cookbook.supervised.common import compute_mean_nll
                # Get logprobs for SFT data only (need to recompute for metrics)
                sft_forward_future = await training_client.forward_async(sft_data, loss_fn="cross_entropy")
                sft_forward_result = await sft_forward_future.result_async()
                sft_logprobs = [output["logprobs"] for output in sft_forward_result.loss_fn_outputs]
                sft_weights = [datum.loss_fn_inputs["weights"] for datum in sft_data]
                sft_nll = compute_mean_nll(sft_logprobs, sft_weights)
                metrics[f"train/sft_nll"] = sft_nll
        else:
            # Regular training - all data uses same loss function
...
async def do_sync_training(
    ...
    """Implements fully synchronous on-policy training with optional GRPO critique training"""
    # Initial sampling client
    sampling_client, _ = await save_checkpoint_and_get_sampling_client(
        training_client, start_batch, cfg.log_path, cfg.save_every, start_batch
    )

    # Initialize SFT buffer if enabled
    sft_buffer: deque[tinker.Datum] = deque(maxlen=cfg.sft_buffer_size) if cfg.sft_buffer_size > 0 else deque()
    ...
    for i_batch in range(start_batch, end_batch):
        ...
        # Run evaluations
        if cfg.eval_every > 0 and i_batch % cfg.eval_every == 0 and not (i_batch == 0 and cfg.skip_first_eval):
            with timed("run_evals", metrics):
                eval_metrics = await run_evaluations_parallel(
                    evaluators, sampling_client, cfg, i_batch
                )
        ...
        if hasattr(dataset, 'update_sampling_client'):
            dataset.update_sampling_client(sampling_client)
        # Use regular RL dataset
        env_group_builders_P = dataset.get_batch(i_batch)
        metrics["train/is_critique_batch"] = 0.0
...
async def main(
    ...
    elif cfg.start_batch_index > 0:
        start_batch = cfg.start_batch_index
    else:
        start_batch = 0
    ...
    if maybe_test_dataset is not None:
        if isinstance(maybe_test_dataset, list):
            # Multiple test datasets with custom names
            for test_name, test_dataset in maybe_test_dataset:
                evaluators.append(RLTestSetEvaluator(
                    test_dataset,
                    max_tokens=cfg.max_tokens,
                    name=test_name,
                ))
        else:
            # Single test dataset with default name "test"
            evaluators.append(RLTestSetEvaluator(
                maybe_test_dataset,
                max_tokens=cfg.max_tokens,
